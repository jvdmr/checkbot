...
sub weHaveSaidThisTooManyTimesAlready {
    ...
    if ($count >= $recentMessageCountThreshold and
        ...
        $$do = 'me';
        $$msg = "was going to say '$text' but has said it too many times already";
...
sub getURI {
    my $self = shift;
    my ($event, $uri, @data) = @_;
    $self->spawnChild($event, 'wget', ['--quiet', '--passive', '--user-agent=Mozilla/5.0 (compatible; mozbot)', '--header=Accept-Language: nl,en', '--output-document=-', $uri], 'URI', [$uri, @data]);
}
